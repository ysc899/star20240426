...
//        PhotoViewAttacher(iv)
        ...
        AndroidUtil.log("사진 fileName :  " + fileName)

        val tvTotal = view.findViewById<TextView>(R.id.tvAdGalleryTotal)
        val tvSend = view.findViewById<TextView>(R.id.tvAdGallerySend)


        val sDate = DateUtil.getDate(today, "yyyyMMdd")
        val tDate = DateUtil.getDate(DateUtil.getFormatString(Date(), "yyyyMMdd"), "yyyyMMdd")

        val sTime = sDate.time
        val tTime = tDate.time
        var isSend = true

        view.findViewById<Button>(R.id.btReSend).visibility = View.VISIBLE

        if (sTime < tTime) {
            isSend = false
            view.findViewById<Button>(R.id.btReSend).visibility = View.GONE
        }

        //AndroidUtil.log("isSend : " + isSend)
    ...
    companion object {
        ...
        fun create(image: File) = PhotoFragment().apply {
            arguments = Bundle().apply {
                ...
                //AndroidUtil.log("사진 : " + image.absolutePath)
            }
        }
