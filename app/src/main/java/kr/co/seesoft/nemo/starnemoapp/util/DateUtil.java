...
public abstract class DateUtil {
	...
	public static String getSysDate(final String strFormat) {
		// Date클래스에서 현재의 날짜 시간을 가져옴
		Date datetime = new Date();
		SimpleDateFormat formattime = new SimpleDateFormat(strFormat);
		String texttime = formattime.format(datetime);
	...
	public static long dayDiff(final String startDate, final String endDate, final String format) throws Exception {
		...
		try {
			...
			day2day = (eDate.getTime() - sDate.getTime()) / 1000; // 초
		} catch (Exception e) {
	...
	/**
	 * @param date date
	 * @param hour 더하거나 뺄 시간
	 * @return 더하거나 빼진 시간
	 */
	public static Date addHour(final Date date, final int hour) {
		Calendar cal = Calendar.getInstance();
//		String formatApply= "yyyy-MM-dd HH:mm:ss.SSS";
		cal.setTime(date);
